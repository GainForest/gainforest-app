...
**Routable vs public locales.** `SUPPORTED_LOCALES` (`lib/i18n/languages.ts`) is every locale that routes; `PUBLIC_LOCALES` is the subset that is complete enough to advertise, and it is derived from `SUPPORTED_LANGUAGES` so "appears in the language picker" and "is indexable" can never drift apart. Only public locales are emitted as `hreflang` alternates, listed in the sitemap, or offered in the picker — a part-translated locale still routes for testing but is never surfaced to search engines as duplicate English. Arabic (`ar`) is currently routable but not public.

**Partially translated locales.** `messages/locales.ts` layers a partial catalog over the complete English one via `mergeMessages`, so a missing key falls back to English instead of throwing (`i18n/request.ts` hands next-intl one tree and a missing key is a runtime error, not a blank). Ship only the namespaces that are genuinely translated; never copy English into a locale file to satisfy the shape, because copied English is exactly what `pnpm check:i18n` flags. Register such a locale in that script's `partialLocales` set, which relaxes only the key-completeness rules — anything the locale *does* define must still be real translation. Content authored per locale in code rather than in `messages/` (email templates) is keyed by the narrower `PublicLanguageCode` and narrowed with `resolvePublicLanguage()`.

**Right-to-left.** Arabic is the first RTL locale. `dir` is set from the locale on `<html>` in `app/layout.tsx` and kept in step client-side by `LanguageSelector`. Layout must use logical Tailwind utilities (`ms-`/`me-`/`ps-`/`pe-`/`start-`/`end-`/`text-start`/`border-s`/`rounded-s`) rather than physical ones (`ml-`/`mr-`/`left-`/`text-left`); logical utilities resolve to the same value under LTR, so they are a no-op for the other locales. The one deliberate exception is the centering idiom `left-1/2 -translate-x-1/2`, which must stay physical because `translate-x` does not flip with direction. `scripts/codemod-logical-properties.mjs` performs and re-checks this conversion and encodes those safety rules; run it after adding markup with physical direction classes.
