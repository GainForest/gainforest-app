...
export function CheckoutView({
  ...
  const cardsMessage = !authSession.isLoggedIn ? t("signedOutNote") : anonymous ? t("anonymousHint") : null;
    ...
    <div className="mx-auto w-full max-w-5xl px-4 py-8">
      ...
      <div className="mt-8 grid gap-x-12 gap-y-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start">
        ...
        <div className="min-w-0">
            ...
            {tipEnabled && tipUsd > 0 ? (
              <li className="flex items-center justify-between gap-4 py-4">
                <span className="flex min-w-0 items-center gap-2.5">
                  ...
                  <span className="min-w-0">
                    <span className="block truncate text-sm text-foreground">{t("tipLineLabel")}</span>
                    <span className="block truncate text-xs text-muted-foreground">GainForest</span>
                  </span>
                </span>
                <span className="shrink-0 tabular-nums text-sm text-foreground">${tipUsd.toFixed(2)}</span>
              </li>
            ) : null}
          </ul>

          {blockedItems.length > 0 ? (
            <p className="mt-4 rounded-2xl bg-amber-500/10 px-3 py-2 text-xs leading-5 text-amber-700 dark:text-amber-400">
              {t("blockedItems", { titles: blockedItems.map((item) => item.title).join(", ") })}
            </p>
          ) : null}

          <div className="mt-5 space-y-1.5 border-t border-border-soft pt-5">
          ...
          {/* Options + CTA follow the totals, still in the main column */}
          <div className="mt-6 divide-y divide-border-soft border-t border-border-soft">
            {authSession.isLoggedIn ? (
              <section className="py-5">
                <label className="flex cursor-pointer items-center justify-between gap-4">
                  ...
                  <Checkbox checked={anonymous} onCheckedChange={(checked) => setAnonymous(checked === true)} disabled={paying} />
                </label>
              </section>
            ) : null}
            ...
            <section className="py-5">
              <Textarea
                ...
                className="min-h-20 resize-none rounded-xl border-0 bg-muted/40 shadow-none focus-visible:bg-muted/60 dark:bg-muted/40"
              />
            ...
            <section className="py-5">
              {wallet ? (
                ...
                <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-3">
                  <p className="min-w-0 text-sm text-muted-foreground">{t("walletHint")}</p>
            ...
            {tipEnabled ? (
              <section className="py-5 last:pb-0">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-sm tabular-nums text-muted-foreground">${tipUsd.toFixed(2)}</span>
                </div>
          ...
          {wallet && wallet.balance !== null && !hasEnoughBalance ? (
            <p className="mt-5 rounded-2xl bg-destructive/10 px-3 py-2 text-sm text-destructive">
        ...
        <aside className="lg:sticky lg:top-24">
          <div className="rounded-3xl bg-muted/40 p-5">
            {willEarnCards ? (
              ...
              <p className="text-sm leading-6 text-muted-foreground">{cardsMessage ?? t("cardsPreviewTitle")}</p>
            )}
          </div>
        </aside>
      </div>
