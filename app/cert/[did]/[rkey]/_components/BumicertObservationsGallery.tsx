...
export function BumicertObservationsGallery({
  observations,
}: {
  ...
  useEffect(() => {
    ...
    setDrawer((current) =>
      current
        ? (observations.find((item) => item.atUri === current.atUri) ?? null)
        : null,
    );
  }, [observations]);
    ...
    <section className="mt-10 border-t border-border-soft pt-6">
      ...
      <RecordDrawer
        ...
        onRecordUpdated={(record) => {
          ...
          setItems((current) =>
            current.map((item) =>
              item.atUri === record.atUri ? record : item,
            ),
          );
        }}
        onRecordDeleted={(record) => {
          setDrawer(null);
          setItems((current) =>
            current.filter((item) => item.atUri !== record.atUri),
          );
        }}
      />
    </section>
