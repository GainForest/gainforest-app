...
import { ArrowRightIcon, FlaskConicalIcon, GlobeIcon, HeartHandshakeIcon, LeafIcon, MailCheckIcon, ShieldCheckIcon, SparklesIcon, TrophyIcon } from "lucide-react";
...
export default async function TestRegistryPage() {
  ...
  const rewildingMilestones = await getTranslations("cart.testRegistry.rewildingMilestones");
        ...
        <section className="mt-10">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            >
              ...
              <h3 className="mt-8 text-xl font-semibold text-foreground">{invitationDelivery("cardTitle")}</h3>
            ...
            <Link
              href="/_test/rewilding-milestones"
              className="group flex min-h-60 flex-col rounded-[2rem] border border-border-soft bg-surface p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="grid size-12 place-items-center rounded-2xl bg-primary/10 text-primary">
                  <LeafIcon className="size-6" aria-hidden />
                </div>
                <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">
                  {rewildingMilestones("mockBadge")}
                </span>
              </div>
              <h3 className="mt-8 text-xl font-semibold text-foreground">{rewildingMilestones("cardTitle")}</h3>
              <p className="mt-2 text-sm leading-6 text-muted-foreground">{rewildingMilestones("cardDescription")}</p>
              <span className="mt-auto inline-flex items-center gap-1.5 pt-6 text-sm font-semibold text-primary">
                {t("openExperience")}
                <ArrowRightIcon className="size-4 transition-transform group-hover:translate-x-1" aria-hidden />
              </span>
