...
import { AudioWaveformIcon, InfoIcon } from "lucide-react";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    // Admin-only while the soundscape clock is being iterated on: keep it out
    // of search results and out of app/sitemap.ts.
    robots: { index: false, follow: false },
  };
}

export default async function SoundscapePage() {
  // Gated to members of the GainForest admin group (any role). Hiding the
  // sidebar entry is cosmetic — this server-side check is the real gate.
      ...
      <header className="max-w-2xl">
        <div className="flex items-center gap-3">
          <span className="flex size-9 items-center justify-center rounded-full border border-primary/15 bg-primary/[0.08] text-primary shadow-inner">
            <AudioWaveformIcon className="size-4.5" />
          </span>
