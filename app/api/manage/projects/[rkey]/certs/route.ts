...
async function directProjectFromRecord(did: string, record: ListedRecord): Promise<ManagedProject | null> {
  ...
  return {
    ...
    bumicertUris: items.filter((item) => item.uri.includes("/org.hypercerts.claim.activity/")).map((item) => item.uri),
    bumicertCount: items.filter((item) => item.uri.includes("/org.hypercerts.claim.activity/")).length,
    datasetUris: items.filter((item) => item.uri.includes("/app.gainforest.dwc.dataset/")).map((item) => item.uri),
