...
const FIELD_DOCS: FieldDoc[] = [
  ...
  {
    field: "root_collar_diameter",
    description: "Root collar diameter (cm)",
    format: "Number",
    helperText: "Useful for planted or young trees where trunk diameter is not yet meaningful.",
  },
  { field: "diameter", description: "Older files can also use this heading for root collar diameter", format: "Number" },
            ...
            <div className="divide-y divide-border">
              {FIELD_DOCS.map((doc) => (
                <div key={doc.field} className="grid grid-cols-[1fr_1.5fr_0.6fr] items-start gap-0 px-4 py-2.5">
                  <span className="font-mono text-sm text-foreground">{doc.field}{doc.required && <span className="ms-1 text-destructive">*</span>}</span>
