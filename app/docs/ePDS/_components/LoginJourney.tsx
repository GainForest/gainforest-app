...
type NodeId = "app" | "pds" | "auth" | "inbox";
...
// Which node the login "packet" sits at, and which edge lights up, per step.
const STEP_FLOW: { at: NodeId; edge?: [NodeId, NodeId] }[] = [
  { at: "app" },
  { at: "pds", edge: ["app", "pds"] },
  { at: "auth", edge: ["pds", "auth"] },
  { at: "inbox", edge: ["auth", "inbox"] },
  { at: "auth", edge: ["inbox", "auth"] },
  { at: "auth" },
  { at: "pds", edge: ["auth", "pds"] },
  { at: "app", edge: ["pds", "app"] },
  { at: "app" },
];
...
// The centerpiece of the explainer: a step-through animation of the whole
// email-code login round trip. A dot travels between the four participants
...
export function LoginJourney() {
  ...
  return (
    <div>
      <svg viewBox="0 0 720 300" className="mx-auto block w-full" style={{ maxWidth: 640 }} role="img" aria-label={t("ariaLabel")}>
        {EDGES.map(([a, b]) => {
