...
## Metrics
Current phase:
- **Primary**: `observation_sitemap_discovery_gaps` (count, lower is better) — high-quality public observation detail pages missing sitemap discovery now that their metadata and structured data are complete.
- **Secondary**: `observation_breadcrumb_gaps` — dynamic observation detail pages missing BreadcrumbList JSON-LD for search-result hierarchy.
...
- Reduced `project_breadcrumb_gaps` from 1 to 0 by adding `BreadcrumbList` JSON-LD to dynamic project detail pages.
...
- Reduced `projects_server_content_gaps` from 1 to 0 by adding cursor-aware initial page support to `ProjectsExploreClient` and fetching a default first project page on the server.
- Started an eighteenth phase for crawlable organizations content: `/organizations` is a public sitemap-backed index with metadata/social/structured data, and should server-render the first page of organization cards while preserving filter and load-more behavior.
- Reduced `organizations_server_content_gaps` from 1 to 0 by adding cursor-aware initial page support to `OrganizationsClient` and fetching a default first organization page on the server.
...
- Reduced `observation_breadcrumb_gaps` from 1 to 0 by adding BreadcrumbList JSON-LD to dynamic observation detail pages.
- Started a twenty-second phase for observation sitemap discovery: high-quality public observation detail pages are now complete landing pages, so recent image-backed sightings should be discoverable in sitemap.xml without trying to list every private or low-value app surface.
