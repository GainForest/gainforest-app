...
pg_isready -q || { echo "local disposable PostgreSQL did not become ready" >&2; exit 1; }
...
PUBLIC_CRYPTO_ENQUEUE=$(psql -X -Atq -v ON_ERROR_STOP=1 -d "$PUBLIC_CRYPTO_DB" -c "select status || '|' || duplicate from public.notification_outbox_enqueue('public-pgcrypto:enqueue','signup','{}','public-pgcrypto-source',null,'public-pgcrypto@example.com','welcome','en','public-pgcrypto-source','capture',clock_timestamp());")
...
select outbox_id from public.notification_outbox_enqueue(
  ...
  'race@example.com', 'race-template', 'en', 'race-source', 'resend', clock_timestamp()
);
SQL
)
