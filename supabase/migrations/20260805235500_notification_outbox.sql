...
create function public.notification_invitation_create(
  ...
  p_delivery_mode text,p_created_at timestamptz,p_expires_at timestamptz
...
begin
  ...
  if p_delivery_mode is not null and p_delivery_mode not in ('capture','resend') then raise exception 'invitation delivery mode must be capture, resend, or null'; end if;
...
begin
  ...
  elsif v.status='dead' and v.last_error_code='provider_rejected' and v.redacted_at is null and v.provider_call_phase='idle' then
...
returns jsonb language sql stable security definer set search_path='' as $$
  select pg_catalog.jsonb_build_object(
    ...
    'dead',count(*) filter (where status='dead' and redacted_at is null),
